...
- **對帳單匯入（v0.7.0+）**：CSV / Excel 拖放，自動偵測台股 Top 5 銀行 / 券商格式，重複交易標記與處置，並已延伸到匯入歷史、rollback 與對帳工作台
- **正式報表與匯出（v0.11.0）**：損益表、資產負債表、現金流量表，支援 PDF / CSV 匯出
- **投資績效分析（v0.12.0）**：XIRR、TWR / MWR、benchmark 對比、損益歸因
- **風險分析（v0.13.0）**：年化波動率、最大回撤、Sharpe Ratio、集中度分析
