...
兩個大改動：新增／編輯交易的對話框全面重新設計，以及信用卡從「負債」改為「付款方式」。

### 新增／編輯交易對話框

- **類型改用 chips**：買入／賣出等交易類型從下拉選單改為一排可點的 chips，常用類型直接露出、平鋪呈現，不再藏進「更多」popup，一眼看到目前選了哪個。
- **成交明細改三欄（數量 ｜ 每股價格 ｜ 成交總額）**：三欄並排、密度對齊常見看盤軟體；手續費併入同一列，整個「進階」Expander 移除，少一層展開。
- **跨幣別買賣統一走「匯率估算」**：移除原本複雜的「結算明細卡／扣款依據」切換，跨幣別（如複委託美股由台幣帳戶扣款）一律以當日市場匯率估算台幣扣款，實際入帳金額仍可在賣出時填寫。介面大幅精簡。
- **欄位說明收進 ⓘ 提示**：說明改為滑過 label 旁 ⓘ 顯示，版面更乾淨。
- **投資標的隱藏「交易幣別」下拉**：幣別已顯示於資產列，不再重複要求選一次。

### 帳戶連動更嚴謹

- **勾了「從帳戶扣款 / 存入帳戶」就必須指定帳戶**：買入／賣出／收入／現金股利若勾了要連動現金卻沒選帳戶，過去會靜默存一筆「不動任何餘額」的交易，讓現金餘額與淨值悄悄對不上。現在會擋下並提示選帳戶；取消勾選＝刻意不連動，仍可不選帳戶。

### 信用卡

- **信用卡改為「付款方式」，退出負債與淨值**：先前「每月建立信用卡負債再消帳」的流程並不直覺。信用卡不再算負債、不計入淨值，改為一個「付款方式」標籤供交易標註用哪張卡；既有卡片自動搬遷。
- **舊「刷卡／繳卡費」流程退役**：不再新增這兩種交易；歷史的刷卡／繳卡費紀錄設為不可編輯（避免開出空白對話框），需要移除請直接刪除該筆。

### 其他

- 消除全站 ComboBox 因多餘 `DisplayMemberPath` 造成的 ContentTemplate/Selector 樣板警告。

---

## v0.38.0 - 2026-07-14

匯率一直刷不了的**真正根因修法**——把即時與歷史匯率的來源整批換成 Yahoo Finance。

### 匯率

- **匯率終於能真正刷新——來源改用 Yahoo Finance**：先前「匯率長期用預設值、每次開機都抓不到」的真正原因，是**台灣銀行的匯率 CSV 端點被套上反爬蟲挑戰頁**（回一頁 HTML 而非 CSV），無論重試幾次都解不出來（v0.37.0 加的重試因此沒對到根因）。即時匯率改為逐一向 Yahoo 抓各外幣對台幣（與個股報價同源、免金鑰），匯率能正常更新，外幣（美股等）估值不再長期卡在預設值。
- **歷史匯率回補一併改走 Yahoo**：多幣別歷史估值所需的歷史匯率，原本也是抓同一個被擋掉的台銀端點、靜默拿不到資料；改用既有的 Yahoo 歷史來源後能正常回補。

---

## v0.37.0 - 2026-07-14

一批穩定性與正確性修正——每日損益補齊、匯率真的會刷新，以及數個背景/綁定雜訊的清理。

### 投資資產 / 績效比較

- **每日損益、走勢不再因為缺當日價而整天留白**：某天只要有一檔持股還沒抓到當日收盤（多市場交易日／資料新舊不一致，例如台股 ETF 收盤比美股晚到），該天原本會被整天跳過。現在改用「窗內最近一筆收盤」往前補（forward-fill），沒開 App 的日子也補得齊；仍保留「連續多天都沒任何價才留白」的防呆。
- **績效比較色點**的一個 XAML 綁定錯誤已修（改用轉換器繫結，不再噴 binding 錯誤）。

### 匯率

- **啟動時的即期匯率抓取加入重試**：開機初期網路／DNS 常還沒就緒，導致每次開機的匯率抓取失敗、一直退回快取／預設值。現在會重試幾次（遞增間隔）等網路就緒後補上，匯率能真正刷新。

### 穩定性 / 背景

- **消除同步狀態計數器造成的例外風暴**：待推送計數器對一張表名寫錯（分類），每次計數都丟出並吞掉一個資料庫例外；改用結構查詢預先過濾，不再拿例外當流程。
- 修掉多個 XAML 綁定錯誤，並加上單一實例防護（避免同時開兩份互搶資料庫／設定檔）。

---

## v0.36.0 - 2026-07-05

投資組合群組（自建投資組合）在績效比較與走勢圖的三個計算修正——多幣別（含美股）群組現在算得對了。

### 績效比較 / 投資資產

- **群組報酬線不再暴跌 −100%**：交易時間以 UTC 儲存（台灣午夜＝16:00Z），群組報酬計算的「持倉日期」用 UTC 日、「現金流日期」用本地日，兩者差一天 → 買進的現金流比持倉晚一天被扣、報酬公式崩到 −100%。持倉日期改用與現金流同一套（本地日）對齊。
- **群組市值走勢不再漏算 ETF**：走勢圖的資料來源（sparkline）原本只載入 `AssetType.Stock` 的部位，會漏掉 ETF／基金／債券…——例如台股 ETF 00988A 就整檔被漏掉、走勢金額短計。改為涵蓋所有可交易標的。
- **群組市值／報酬改為換匯計算**：群組的市值與現金流原本完全不換匯，多幣別群組（含美股）把外幣原數當台幣直接相加 → 現值嚴重短計、報酬率權重失真。現在用交易上的幣別與匯率一致換算成台幣。
...
- **無法顯示的比較項目不再靜默消失**：把某項目加入比較（如自建投資組合群組）時，若這個區間畫不出線（群組買賣不成對、查無代號價格、盤中區間不適用、投組快照不足…），原本 token 會被默默存起來卻沒有任何 chip、沒有線、沒有提示，看起來就像「根本加不進去」。現在改成顯示一顆**可移除、滑上去看原因的灰色 ⚠ 標籤**，並依情況給不同說明。
- **所選項目全都畫不出時，圖區顯示說明**：不再露出一張空白格線圖＋怪座標軸，改為置中顯示「所選項目目前都無法比較，滑到上方 ⚠ 標籤看原因」。
- **移除全部比較項目時**不再殘留幽靈標籤。
...
以「上手友善化」為主軸——讓第一次開啟的使用者更容易知道從哪裡開始，加上一條財務概覽金額的重要修正。

### 上手引導 / 導覽

- **導覽拆「核心 / 進階」群組**：預設只展開核心區（資產核心、收支、分析），進階區（其他資產、規劃、工具）收合，第一次使用不再被整排項目淹沒；**展開偏好會記住**。
- **首次開啟顯示 Welcome 引導**：一張歡迎卡帶你加第一筆持股、看到損益，之後不再出現。
- **導覽項目加白話 tooltip**：滑過即說明這頁在做什麼，不用先點進去猜。
- **首頁與全站空狀態統一**：沒有資料時顯示友善的引導卡＋主要行動按鈕（從這裡開始加持股／記一筆／建分類），取代原本的空白面板。

### 修正

- **財務概覽「投資」彙總改用已換匯的 base 市值**：修正美股（複委託）在查無當日匯率時被以原幣面額當成台幣加總，導致概覽淨值嚴重短計、與投資組合頁的總市值對不上的問題。

---

## v0.33.0 - 2026-07-01

以「績效比較頁的盤中分時重構」為主軸，加上賣出交易對齊買入、跨幣別匯率的多處修正與防呆，以及持股／帳戶清單的可讀性改善。

### 績效比較（資產趨勢 vs 大盤）

- **1D／5D 改吃盤中分時**（Fugle／Yahoo），畫真實盤中曲線，取代原本只有 2 點的斜線。
- **跨市場盤中改 Google 式真實時間軸**：台股白天、美股半夜各自錯開；5D 壓縮非交易時間（移除週末／假日 gap）。
- **Google 式 hover**：滑鼠十字準星＋貼齊資料點的線上實心圓點＋讀數框（日期時間＋每項現值＋同期 %）；盤中一律只顯示時間。
- **圖表與對標清單合併成一張卡**，去掉中間空隙；chips 加底色／加大、清單加列分隔線／粗體／漲跌箭頭。
- **比較搜尋納入美股**；混市場盤中資料修正（釘住 Y 軸、`@組合／@投組` 不再整張退日線、盤中補 User-Agent 修 Yahoo 節流）。

### 交易

- **賣出對齊買入**：新增「成交總額」輸入模式（GROSS，反推單價）＋「進階」收合區（手續費／帳戶／跨幣別）；跨幣別賣出自動展開進階，避免必填的結算／匯率欄位被藏住。
- **新增標的「代號」自動完成**（台股＋美股），點選帶入名稱／幣別／類型。
- **持股數量去尾零**：修 ETF／碎股顯示「200,000.0000」。

### 跨幣別 / 匯率

- **「依帳戶明細」模式也能用自動匯率反推單價**：修複委託只填台幣扣款卻卡「跨幣別反推單價需要匯率」。
- **買入確認前同步補抓當日匯率**：消除「設好日期就按確認、匯率還沒回來」的競態。
- **同幣別交易持久化前一律清掉 FX 欄位**（防呆）：杜絕同幣別交易被塞入 ≠1 匯率而污染現金流／成本的資料問題。

### 資金帳戶

- **交易清單直接顯示資產名稱**：買入／賣出／股利顯示「名稱（代號）」，原本要點進去才看得到。

---

## v0.32.0 - 2026-06-18

投資資產頁的「入口正名 + 持股表重整 + 欄內視覺層級」為主軸，加上資金帳戶分類、ETF 偵測與外幣彙總的修正，並把投資組合管理改為對話框、補上多處使用者偏好記憶。

### Investment portfolio page

- **入口正名分層**：頂部主鈕「＋ 新增交易」、工具列「加入觀察」、分頁列「管理組合」，消除兩個同名又不同功能的「＋ 投資」按鈕。
- **卡片「概覽」收合鈕正名**：原本掛著組合名「全部」易被誤認為換組合下拉，改為直白的「概覽」。
- **持股表重整為「詳細但去冗餘」**：幣別折進資產欄（僅外幣顯示）、淨值折進市值欄第二行；數量／成本均價／付出成本維持各自獨立、可逐欄排序。
- **欄內視覺層級**：參考欄（數量／成本均價／付出成本）轉淡、淨損益放大為整列主角，密集表也好掃。
- **資產側邊面板「＋ 新增交易」對 ETF／基金等非 Stock 部位也顯示**（原本只認 `AssetType.Stock`，ETF 部位 badge 顯示「股」卻沒有按鈕）。

### Cash accounts

- 「存入」交易不再顯示／儲存支出分類（外部資金搬入、非收支，與轉帳同性質）；只有「提款」帶支出分類。

### Portfolio management

- 投資組合管理改為 shell 模態對話框（取代沒有出口的整頁），支援就地新增／編輯；移除整頁路由死碼。

### Preferences

- 記住使用者選擇：「概覽」展開/收合、「顯示已平倉」、走勢圖期間、月結報告年月、報酬日曆檢視、FIRE 路徑分頁。

### Fixes

- **持股表頁尾外幣彙總失真**：原本把各列原幣金額直接相加而未換算，含外幣部位時市值／成本／損益全失真；改用 base 幣別換算後加總，與頂部卡片一致。
- App 工作列／檔案總管圖示改用 BMP 編碼的 `.ico`，修正顯示通用圖示。

---

## v0.31.0 - 2026-06-17

投資資產頁的版面收斂與一個資料正確性修正。

### Fixes

- **修正「移至投資組合」靜默不寫入 DB**：把持股指派到自訂投資組合，當下生效但重啟後跑回預設群組。根因是 `PortfolioViewModelFactory` 漏接 `PositionMetadata` 服務，導致更新走到 Null no-op；已補上注入並加防迴歸測試。
- 投資組合分頁選取 pill 底部被裁切修正（分頁列高度／內距與 ListBox padding 調整）。

### Investment portfolio page

- 上方概覽瘦身：標題卡改無邊框貼頁、今日漲跌對齊、焦點卡與 insights chip 間距調整；新增「概覽」一鍵收合，收起走勢圖／焦點卡／insights，只留頭條，讓下方持股表格成為主角。
- 移除上方標題卡與下方 KPI 卡之間的數字重複（市值／損益／今日漲跌），KPI 卡改為精簡 insights chip（持股佔比／成本分布／盈虧檔數／每日損益），下鑽圖表完整保留；卡片樣式統一。
...
- 移除持股列 inline「移至投資組合」按鈕：原本點該按鈕會同時觸發 DataGrid 列選取（開側邊面板）與編輯資產對話框，一鍵冒出兩個畫面；移動／編輯改由側邊面板既有入口操作，表格也更清爽。
- 側邊面板資產徽章（類型／ETF／幣別）改用共享徽章樣式，統一高度與垂直置中，消除「股」與「TWD」高度不一、與同列按鈕未對齊的問題。

---

## v0.30.0 - 2026-06-16

這版以「投資資產頁 Google 式重構」與「新增投資流程改版」為主軸，並把即時／歷史匯率來源改為台灣銀行即期買入，重構設定頁為分類導覽，補上快照歷史重建工具，同時修掉多個會造成閃爍、崩潰或資料口徑不一致的問題。

### Investment portfolio page

- 投資資產頁改為 Google Finance 式版面：頂部投資組合分頁取代舊「檢視模式」，每個組合有獨立詳情標題列（總值／漲跌／趨勢／期間／＋投資）。
- 新增「股票 vs ETF」投資組合焦點卡，依有效 `IsEtf` 旗標分類（不再只看 `AssetType`）。
- 分頁選擇直接驅動既有 `PortfolioGroupFilter`（沿用 `PositionsView`，未另建第二個 CollectionView）。
- 介面文案統一「群組 → 投資組合」（識別碼與資料結構不動）。
- 新增投資組合對話框精簡：移除多餘的「說明／顏色」欄位，改自動配色。
- 卡片在淺色／深色主題都套用明顯邊框（`AppCardBorder`）。

### Add-investment flow

- 買入表單預設精簡為「數量／日期／單價＋自動手續費/稅費摘要」，把價格模式、成交總額、手續費覆寫、資金帳戶與跨幣別覆寫收進可摺疊的「進階」區塊。
- 跨幣別交易時自動展開「進階」，避免漏填匯率資訊；稅費／手續費／匯率能力完整保留，未走多筆 lot。

### Exchange rates

- 即時與歷史匯率來源改用台灣銀行即期買入（即時 day CSV ＋ 歷史月 CSV），取代先前的 Yahoo／Frankfurter（Frankfurter 不支援台幣）。

### Settings

- 設定頁重構為左側分類導覽 ＋ 9 個分類子視圖，便於日後擴充（行為不變）。
- 設定變更即時生效：報價即時刷新、財務總覽 focus／基準幣別、benchmark 不再需要重啟。

### Snapshot history

- 設定頁新增「重建快照歷史」工具：dry-run 預覽 ＋ 自動備份後寫入，重建含 breakdown 與 as-of-D 匯率；僅在當日所有部位皆可定價時才寫入，避免殘缺市值污染。

### Stability and data integrity

- 修正設定變更回饋迴圈造成的全程式閃爍（配額記帳改靜默存檔，報價刷新僅在來源／金鑰變更時觸發）。
- 修正多處背景執行緒直接變更 UI 綁定集合造成的崩潰／競態（AllocationPanel、新增投資組合分頁、Goals 更新）。
- 修正投資資產頁載入崩潰（移除已刪除樣板的遺留 `DataGrid.GroupStyle` 參照）。
- 報酬日曆與資產趨勢正確處理缺 breakdown 的回填日，消除留白與基準跳動。
- 財務總覽報價 tick 改 Rx Sample 節流；負債比率／佔比／總資產 KPI 與 Hero 口徑一致；Goals 不再每 tick 重載。
- 編輯交易正確帶入已平倉／不在清單的標的與類型。
...
- `docs/planning/Assetra-US-Market-Data-Plan.md` 新增美股 market data 導入計畫，明確鎖定 symbol directory + routed quote provider + quota/cache/calendar/FX 邊界。
- Release-facing docs 做 v0.28.0 一致性整理：README、feature roadmap、implementation roadmap、technical blueprint、settings migration 與 deferred roadmap 對齊目前 master 狀態。
- AI assistant 規劃文件對齊 v0.27 shipped baseline，並將舊 `v0.23-Assistant-Sprint-Plan.md` 移到 `docs/archive/sprints/` 作為歷史草案。

---

## v0.27.0 - 2026-05-09

「AI 助手三階段 + 大規模技術債整理 + UX 補強」release。代表 Assetra 接近 v1.0 GA 的最後一波重構衝刺。

### 新功能 — 使用者面

- **多年度稅務比較**（v0.27 T1）— 「報表 → 稅務」頁加入跨年度比較 panel，自動列出有任何稅務記錄的所有年份。
- **AMT 完整稅額計算**（v0.27 T2）— 設定加 `AmtExemption / AmtRate / AmtRegularTaxableIncome / AmtRegularIncomeTax`；報表頁顯示完整公式分解。
- **多幣別匯率即時刷新**（v0.27 T3）— Settings 頁加 Frankfurter 手動刷新按鈕 + 上次更新時間戳 + 各幣別兌台幣表格。`AppSettings.LastFxRefreshUtc` 持久化。
- **Import OCR 預覽**（v0.27 T4）— PDF 解析後加可摺疊面板逐頁顯示 OCR 結果（信心分數 + 文字片段）。
- **AI 財務助手 Phase 1 規則式**（v0.27）— `IFinancialAssistant` + `RuleBasedFinancialAssistant` 處理淨資產 / 現金 / 負債 / 海外所得查詢。
- **AI 助手 Phase 2 主動洞察** — `IAssistantInsightService` + `RuleBasedAssistantInsightService`：預算超支（Critical/Warning）+ 訂閱即將到期（Info）+ 月差異（Warning）。
- **AI 助手 Phase 2 後台排程器** — `AssistantInsightHostedService` 每 4h 推送 Critical/Warning 至 Snackbar。
- **AI 助手 Phase 3 LLM** — `ILlmProvider` 介面 + `NullLlmProvider` / `OllamaLlmProvider` / `OpenAiLlmProvider` + `HybridFinancialAssistant` 規則 → LLM fallback。
- **AI 助手 Phase 3.5 grounded tool-calling** — `IAssistantToolRegistry` + 4 個 tool（net_worth / cash_balances / liabilities / current_month_budgets）pre-fetch 到 system prompt 消除 hallucination。
- **AI 助手對話歷史持久化** — `IAssistantHistoryRepository` SQLite 後端，跨 session 回顧。
- **AI 助手 Settings UI** — Provider 下拉、ApiKey、Model、Endpoint 欄位。
- **AI 助手側欄入口** — 新 `NavSection.Assistant` + `AssistantView`（chat-style + insight cards + 建議查詢 chips）。
- **Tessdata 語言檔下載按鈕** — Settings 頁 OCR 區加按鈕直接開 GitHub releases 頁。

### 重構 — 健康度

- **H1 god-object split** — `TransactionDialogViewModel` 由單體 god-object 拆分為 6 個 sibling sub-VM：
  - `BuyTxViewModel`、`SellTxViewModel`、`DividendTxViewModel`、`LoanTxViewModel`、`TransferTxViewModel`、`CreditCardTxViewModel`
  - 每個自帶 state + Reset() + 對應 PropertyChanged handler
  - XAML bindings 全面遷至 `Buy.X` / `Sell.X` / `Div.X` / `Loan.X` / `Transfer.X` / `CreditCard.X`
  - facade 屬性已刪除，dialog VM 不再有 TxBuy* / TxSell* 等遺留代碼
- **Confirm.cs 拆分** — `TransactionDialogViewModel.Confirm.cs` 從 697 行 → ~234 行（−66%），抽出 6 個 partial：
  - `Confirm.BuySell.cs` / `Confirm.CashFlow.cs` / `Confirm.Dividend.cs` / `Confirm.Loan.cs` / `Confirm.CreditCard.cs` / `Confirm.Transfer.cs`
  - `ConfirmTx` dispatcher 改 strategy-pattern Dictionary 取代 12-arm switch
- **M6-A + M6-B 完整 ObservableCollection 封裝** — Snackbar / ConflictResolution / MainViewModel.SearchResults / Reports.MultiYearTaxRows / Settings.FxRateRows + SupportedCurrencies / Recurring.Subscriptions + Pending / CategoryRow.EditIconOptions / LiabilityRow.ScheduleEntries / FinancialOverview 5 個 / AllocationViewModel.AllocationRows / AssetGroupVm.Items + Portfolio mirror cluster (Positions/Trades/CashAccounts/Liabilities) 全改 `ReadOnlyObservableCollection`，內部寫入路徑統一到 `Internal_*` mutator helpers。
- **M1 Money 邊界遷移** — `IBalanceQueryService` 改回 `Money`、`LiabilitySnapshot` 用 `Money(Money, Money)`、Row VM 加 `BalanceAsMoney` / `MarketValueAsMoney` / `OriginalAmountAsMoney` accessor。
- **L3 Dashboard 解耦** — `IDashboardNavigation` 抽 writeback；`IPortfolioPositionFeed` 擴 10 個 derived totals（NetWorth / DayPnl / TotalAssets 等）為 default interface implementation。
- **H3 Phase 3 test fixture** — `Assetra.Tests/WPF/Fixtures/PortfolioViewModelTestFactory.cs` + `PortfolioVmFixtures.cs` + `FakeTradeRepo.cs`，跨測試類別共用。

### 內部 / 基礎建設

- `TradeSchemaMigrator` 加 `TableExists` guard，修 fresh fixture DB 25 個 baseline failure。
- `FinancialOverviewQueryService` 改 Money end-to-end，orphan liability 用 `Money.Zero(asset.Currency)`。
- `NavRailViewModelTests` 預設改 FinancialOverview。
- 新增 16+ 條 i18n keys（zh-TW + en-US）：Settings.Llm.* / Settings.Ocr.Download / Settings.Section.Amt / Settings.Fx.* / Reports.Tax.* / Reports.Tax.Amt.* / Import.PdfPreview.* / Nav.Assistant / Assistant.* 等。
- 測試從 1119 → 1201 (+82)，全部通過。
...
- Native WPF UI 的過程性 task list 已收斂為 DesignSystem 完成摘要與 UI release gate。
- `docs/INDEX.md` 保留目前仍有效的 Native WPF UI 文件入口。
...
- **文件差距檢查紀錄**：頂部加「⚠️ 多數描述已過時」警示；§三 Goals / Trends 過時差距加刪除線並補「已解決」說明；§四全部重寫為出貨狀態總覽表（匯入 / 績效 / 報表 / Goals / 風險 / 稅務 / 雲端同步全部 ✅），剩餘 Phase 4 待做項明確列出。此歷史紀錄後續已合併進正式架構與 roadmap 文件。
