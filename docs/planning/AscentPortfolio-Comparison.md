...
| P5.8b-prereq-1 | ✅ done | 接 `FetchSellFxRateCommand` + `FetchDividendFxRateCommand` 到 `TransactionFxRateResolver`：mirror Buy 的 `RefreshXxxFxRateAsync` + `SetResolvedXxxFx` + `_suppressXxxFxManualTracking` flag pattern；`OnSellTxChanged` / `OnDividendTxChanged` 補 `InstrumentCurrency` / `CashAccountCurrency` / `SettlementCurrency` 三組 case 觸發 `NotifyCanExecuteChanged` + `QueueXxxRefresh`；FxRate change 加 manual-tracking 邏輯與 Buy 對稱；`OnTxDateChanged` / `OnTxTypeChanged` 同步補 queue 呼叫；`EditTrade` 還原路徑為 Sell / Div 的 FxRate write 加 suppress wrap。SellTxForm / CashDividendTxForm XAML 在 FxRate label 旁加同款 Fetch 按鈕 |
| P5.8b-prereq-2 | ✅ done | Sell + CashDividend `ConfirmAsync` 走 `SettlementInputMode` mode-aware 分支：statement mode 必填 `ActualCashAmount`；fx mode 必填 `FxRate`（implicit 1.0 不再被當成有效值）。錯誤訊息走新 lang key `Portfolio.Tx.Sell.Settlement.Statement.Required` 等對應；之前 P5.8a 故意暫缺的 backend mode-aware 分支補上 |
| P5.8b | ✅ done | 抽 `CrossCurrencyOverlay.xaml/.cs` 共用 UserControl。前置條件已備（資料層 P5.8a 對齊 + 行為層 prereq-1/2 對齊）。UserControl DataContext 期待 sub-VM（Buy / Sell / Div），內層 binding 用無前綴屬性名（`SettlementInputMode` / `IsCrossCurrency` / ...）。4 個 DP 給呼叫方傳差異：`GroupName` / `FetchFxRateCommand` / `ActualCashAmountLabel` / `ActualCashAmountHint`。三個 form 原本各 ~120-180 行 settlement section 全換成 6 行 markup。Buy 的 richer layout（mode toggle 用 Border-radio + statement panel inline 幣別 badge）作 canonical，Sell/Div 自動升級到一致視覺。CashDividendTxForm 補上 `x:Name="Root"` 給 `ElementName` binding 用。5 個 ControlsBehaviorTests 改成 inspect overlay markup + form DP wiring 雙邊驗證 |
...
| P5.11 | ✅ done | Positions 表格 redesign：① 6 個 `.Native` lang key 拿掉「（原幣）」/「(native)」尾綴（成本均價 / 現價 / 淨值 / 付出成本 / 淨損益試算 / 市值）— col 內已有 NT$/US$ 等資料符號 + 獨立幣別 col 表達幣別，header 重複標示反而冗餘；② `Portfolio.AssetWithCurrency`「資產 / 幣別」→「資產」，新增 `Portfolio.Currency`「幣別」；③ `PositionsTabPanel.xaml` DataGrid view：Asset col 拿掉內嵌 currency chip，新增獨立 `Width=70` 幣別 col（`SortMemberPath=CurrencyDisplay`，純文字無背景 + `AppTextSecondary` 弱化）；④ Card view（寬 < 1180px）也把 chip-style Border 換成同樣的 AppTextSecondary 純文字 |
| P5.12 | ✅ done | KPI per-position pie chart 跨幣別配比 bug：使用者新增 DRAM (USD ETF) 後「付出成本分布」donut 沒包含。實際是被加進去了，但 `RebuildPositionPieChartsCore` 直接用 `p.Cost` / `p.MarketValue` / `p.Pnl`（原幣）做 sum 跟 sort，DRAM 12,806 USD（≈ 410K TWD ≈ 5.8% 實際）被當成 0.18% 視覺消失。改用 `CostBase` / `MarketValueBase` / `PnlBase`（`ApplyPositionBaseValuations` 已在 `RebuildTotals` 第一步算好），3 個 pie（marketvalue / cost / pnl）全換。第 4 個 daychange chart 用 `History.Snapshots` 已是 base，不需動 |
| P5.13 | ✅ done | 群組 UI 重構 4 子項：（C1）拿掉 row 2 整列 — 資產類型 + 群組 filter chips 全砍；篩選靠「依類型 / 依群組」view mode 自動分組或 row 4 SearchBox 文字搜尋；VM 端 `SetAssetTypeFilterCommand` / `SetPortfolioGroupFilterCommand` / `PortfolioGroupFilterChips` collection 保留（unit test 覆蓋；`FilterPosition` 的 null-check 永遠 short circuit no-op）。（C2）「+ 新增群組」「管理群組」 buttons 從 `IsGroupViewMode`-gated Grid 拉出來掛到 row 1 DockPanel 右側，所有 view mode 永久可見。（C3）`PortfolioGroupsView.xaml` 建立群組按鈕內層 `AppIcon` + `TextBlock` 補 `Foreground="{DynamicResource Brush.Text.OnAccent}"` 修主色背景上文字對比。（C4）user 拍板保留 full page，不改 modal |
| P5.14 | ✅ done | Allocation 配置概覽視覺 polish：① 右側 holdings table 佔比 col 改用 P5.10 同款 `DonutPercentBadge`（圓圈 + 中央 % 文字 + 識別色）並移到第一欄；② 拿掉資產 col 內 10x10 色塊（donut 已帶識別色不重複）；③ 左側「配置洞察」4 張卡頂部 36x3 `AccentBrush` 圓角色塊 — user 反映「色塊很多餘」視覺很醜 — 拿掉，Grid 從 3-row 縮 2-row；`AccentBrush` DP 留在 VM 不渲染 |
| P5.15 | ✅ done | P5.14 落地後 user 反映 donut 內「51.5%」5-char 文字擁擠 + 圓圈太小 + 文字壓圓弧邊。修法：① `DonutPercentBadge` Width/Height 44 → 56（內部可用 32 → 44）；② `StrokeDashArray ConverterParameter` 28.3 → 37.7 重算 circumference（radius 22 / circ ≈ 138.2 / ÷4）；③ TextBlock `FontSize` Xs → Sm + `Margin="10,0"` 預留 stroke 內側緩衝；④ AllocationView 佔比 col Width 60 → 80、cell `Margin` 8,2 → 8,4、`DataGrid.RowHeight` 52 → 64 配合 badge 56 + margin |
