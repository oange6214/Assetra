...
| P4.9f | ✅ done | 三個 detail panel 的 backdrop 從 `AppDialogOverlayBorder`（dim modal scrim）換成新增的 `AppSidePanelBackdropBorder`（Background=Transparent）。Click-to-close 行為保留（Transparent Border 在 WPF 仍 hit-test 可點）。視覺差異：原本 #66/99 黑色遮罩 → 完全透明，user 可同時看到下層列表做對照（master-detail pattern，貼齊 AscentPortfolio mockup）。其他真實 modal dialog（AddAsset、Confirm 等）仍用 dim scrim — 語意：dim = modal、no dim = side panel |
