...
### 1. 自動化與智慧功能
- AI 財務助理（v0.22.0：自然語言查詢 → 摘要與提醒 → 規劃建議）
- 配息自動入帳
- 異常交易偵測
...
### 3. 多端體驗
- PWA（v0.27.0：read-only 瀏覽器端）
- 行動端（v0.28.0：iOS / Android）
- 推播通知（v0.28.0：帳單到期、目標進度、配息入帳）
- 家庭多帳本（夫妻共用，未排版號）

### 4. 差異化模擬工具
- FIRE / 退休試算（v0.25.0：4% 法則 / SWR）
- 情境模擬（v0.26.0：通膨 / 利率 / 薪資 Monte Carlo）
...
1. **AI 財務助理**（v0.22.0；roadmap 原訂 v0.21.0，因雲端同步 GA 占用版號）
2. **多元資產**（不動產 / 保險 / 退休 / 實物，v0.23 / v0.24）
3. **情境模擬**（FIRE / Monte Carlo，v0.25 / v0.26）
4. **多端體驗**（PWA / 行動端，v0.27 / v0.28）
