...
> **狀態：已完成（2026-04-26）。** F1 / F2 / F3 + D1-2 部分（SubViewModels：Allocation / DividendCalendar / FinancialSummary / SetupNotice）落地。Accounts / Liabilities slice 延到 v0.7。
...
### D1-2 拆 `PortfolioViewModel.cs`（v0.6.0 部分完成）
抽出 section sub-viewmodels（沿用現有 `SubViewModels/` pattern）：
- [x] `AllocationPanelViewModel`
- [x] `DividendCalendarViewModel`
- [x] `FinancialSummaryViewModel`
- [x] `SetupNoticeViewModel`
- [x] `PortfolioHistoryViewModel`（既有，v0.6.0 加入自訂日期範圍）— 取代原規劃的 `NetWorthTrendSectionViewModel`
- [ ] `AccountsSectionViewModel`（延 v0.7）
- [ ] `LiabilitiesSectionViewModel`（延 v0.7）

主 VM 已從 ~2,121 行降到 ~1,900 行；剩餘兩塊風險較高（跨 7 個 XAML 的 `HasNo*` 耦合），建議 v0.6.0 手測穩定後再做。
...
- [x] CI 綠（`dotnet build` 0 errors，`dotnet test` 353 / 353 通過）
- [x] `Assetra.Tests` 覆蓋 F1/F2/F3 的服務層 happy path
- [x] CHANGELOG 補上 v0.6.0 條目
- [x] `docs/architecture/Bounded-Contexts.md` 更新 Goals 標記為「已實作（MVP）」
- [x] `docs/planning/Implementation-Roadmap.md` 勾選對應項目
